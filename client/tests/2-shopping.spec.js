...
test.describe('Nhóm 2: Mua sắm & Khách hàng', () => {
  ...
  test('Tìm kiếm - Hiển thị lỗi khi tìm từ khóa không tồn tại', async ({ page }) => {
    ...
    const searchInput = page.locator('input[type="text"], input[placeholder*="tìm"], input[placeholder*="Search"]').first();
 ...
 test('Giỏ hàng - Tăng số lượng khi bấm thêm vào giỏ', async ({ page }) => {
    ...
    const sanPhamDauTien = page.locator('text=Hồng, text=Lốp, text=Hanh').first();
    ...
    const btnThemGio = page.locator('button:has-text("Thêm vào giỏ"), button:has-text("Mua"), button:has-text("Giỏ hàng")');
    await expect(btnThemGio).toBeVisible({ timeout: 5000 });
    await btnThemGio.click();

    console.log('Đã vào trang chi tiết và click nút thêm vào giỏ hàng thành công.');
  });
  ...
  test('Thanh toán - Không cho phép thanh toán giỏ hàng trống', async ({ page }) => {
    ...
    await page.goto('http://localhost:5173/gio-hang', { waitUntil: 'networkidle' }).catch(() => page.goto('http://localhost:5173/gio-hang'));
    
    // Nút đặt hàng/thanh toán nếu có thì không được cho phép xử lý khi giỏ trống
    const btnThanhToan = page.locator('button:has-text("Thanh toán"), button:has-text("Đặt hàng")');
...
// --- CHỨC NĂNG 9: CHĂM SÓC KHÁCH HÀNG (LIÊN KẾT ZALO) ---
  test('Hỗ trợ - Click biểu tượng chat mở đúng link liên kết Zalo', async ({ page, context }) => {
    ...
    const zaloBubble = page.locator('.fixed, [class*="fixed"], button, div').filter({ has: page.locator('svg') }).last();
    ...
    console.log('Đã kiểm tra luồng: Biểu tượng chat mở đúng tab liên kết Zalo thành công!');
  });
});