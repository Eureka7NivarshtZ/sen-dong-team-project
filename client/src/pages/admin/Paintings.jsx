...
function Paintings({ onNavigate }) {
  ...
  const [paintings, setPaintings] = useState([
    { id: 1, image: "https://picsum.photos/80?1", name: "Blue Sky", category: "Sơn dầu", price: "2,000,000 đ", stock: 12, status: "Còn" },
    { id: 2, image: "https://picsum.photos/80?2", name: "Hoa mùa xuân", category: "Sơn nước", price: "3,500,000 đ", stock: 5, status: "Còn" },
    { id: 3, image: "https://picsum.photos/80?3", name: "Phong cảnh biển", category: "Canvas", price: "4,200,000 đ", stock: 0, status: "Hết" }
  ]);

  const [newPainting, setNewPainting] = useState({ name: "", material: "", price: "", stock: "", image: "" });
  ...
  return (
    <div className="dashboard-container" style={{ display: "flex", width: "100%" }}>
      <Sidebar onNavigate={onNavigate} currentTab="admin-paintings" />
      <div className="dashboard-content" style={{ flex: 1, backgroundColor: "#f4f6f9", minHeight: "100vh" }}>
        ...
        <div style={{ padding: "30px", textAlign: "left" }}>
          <div className="painting-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "25px" }}>
            <h1 style={{ color: "#1c3f3a", margin: 0, fontSize: "24px", fontWeight: "bold" }}>Danh mục tranh</h1>
              ...
              <tbody>
                {filteredPaintings.map((item) => (
                  <tr key={item.id} style={{ borderBottom: "1px solid #f0f0f0" }}>
                    <td style={{ padding: "12px" }}><img src={item.image} alt={item.name} style={{ width: "50px", height: "50px", borderRadius: "6px", objectFit: "cover" }} /></td>
                    ...
                    <td style={{ padding: "12px" }}>
                      <span style={{ color: item.status === "Còn" ? "#27ae60" : "#e74c3c", fontWeight: "bold" }}>{item.status}</span>
                    </td>
      ...
      {showModal && (
        <div className="modal-overlay" style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", backgroundColor: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 2000 }}>
          <div className="modal" style={{ backgroundColor: "#fff", padding: "30px", borderRadius: "10px", width: "400px" }}>
            ...
            <input type="text" placeholder="Tên tranh" value={newPainting.name} onChange={(e) => setNewPainting({ ...newPainting, name: e.target.value })} style={{ width: "100%", padding: "10px", marginBottom: "12px", borderRadius: "6px", border: "1px solid #ccc", boxSizing: "border-box" }} />
            <input type="text" placeholder="Chất liệu / Loại tranh" value={newPainting.material} onChange={(e) => setNewPainting({ ...newPainting, material: e.target.value })} style={{ width: "100%", padding: "10px", marginBottom: "12px", borderRadius: "6px", border: "1px solid #ccc", boxSizing: "border-box" }} />
            <input type="text" placeholder="Giá tiền" value={newPainting.price} onChange={(e) => setNewPainting({ ...newPainting, price: e.target.value })} style={{ width: "100%", padding: "10px", marginBottom: "12px", borderRadius: "6px", border: "1px solid #ccc", boxSizing: "border-box" }} />
            <input type="number" placeholder="Số lượng tồn kho" value={newPainting.stock} onChange={(e) => setNewPainting({ ...newPainting, stock: e.target.value })} style={{ width: "100%", padding: "10px", marginBottom: "12px", borderRadius: "6px", border: "1px solid #ccc", boxSizing: "border-box" }} />
            <input type="text" placeholder="Đường dẫn hình ảnh URL" value={newPainting.image} onChange={(e) => setNewPainting({ ...newPainting, image: e.target.value })} style={{ width: "100%", padding: "10px", marginBottom: "20px", borderRadius: "6px", border: "1px solid #ccc", boxSizing: "border-box" }} />
            <div className="modal-buttons" style={{ display: "flex", gap: "10px", justifyContent: "flex-end" }}>
              ...
              <button className="save-btn" onClick={handleSavePainting} style={{ padding: "8px 16px", backgroundColor: "#1c3f3a", color: "#fff", border: "none", borderRadius: "4px", cursor: "pointer" }}>Lưu</button>
            </div>
          </div>
        </div>
      )}
