...
function Home({ products }) {
  ...
  return (
    ...
    >
      ...
      {/* ================= KHỐI 3: PHỔ BIẾN (HIỂN THỊ SAO THẬT) ================= */}
      <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "60px 20px" }}>
        ...
        >
          {productList.length > 0 ? (
            productList.map((product) => {
              ...
              return (
                <div key={product.id} style={{ display: "flex", flexDirection: "column", textAlign: "left" }}>
                  ...
                  {/* 🌟 ĐÃ THÊM: Hiện điểm đánh giá tổng hợp thật ngay dưới Card */}
            ...
            <div style={{ gridColumn: "1 / -1", textAlign: "center", padding: "40px" }}><p>Không có sản phẩm nào</p></div>
          )}
        </div>
      </div>

      {/* ================= KHỐI 4: ĐÁNH GIÁ TỪ FIGMA ================= */}
      <div style={{ backgroundColor: "#edf5f1", padding: "60px 20px", textAlign: "center" }}>
        <h2 style={{ color: "#1c3f3a", fontSize: "28px", fontWeight: "bold", margin: "0 0 8px 0" }}>Đánh giá</h2>
        <p style={{ color: "#666", fontSize: "14px", margin: "0 0 40px 0" }}>Một số đánh giá tích cực từ những khách hàng hài lòng của chúng tôi</p>

        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "30px", maxWidth: "1200px", margin: "0 auto" }}>
          {mockReviews.map((review) => (
            <div key={review.id} style={{ backgroundColor: "#fff", borderRadius: "8px", padding: "35px 25px", display: "flex", flexDirection: "column", alignItems: "center", boxShadow: "0 4px 15 rgba(0,0,0,0.01)" }}>
              <img src={review.avatar} alt={review.name} style={{ width: "60px", height: "60px", borderRadius: "50%", objectFit: "cover", marginBottom: "12px" }} />
              <div style={{ fontSize: "14px", color: "#ffb400", marginBottom: "15px" }}>{review.stars}</div>
              <p style={{ fontSize: "14px", color: "#444", lineHeight: "1.6", margin: "0 0 15px 0", textAlign: "center", flex: 1 }}>{review.comment}</p>
              <h4 style={{ fontSize: "14px", fontWeight: "bold", color: "#555", margin: 0 }}>{review.name}</h4>
            </div>
          ))}
        </div>
      </div>
